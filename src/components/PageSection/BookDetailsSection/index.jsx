...
export default function BookDetailsSection({
  ...
  const onSubmit = () => {
    try {
      const bookIdValue = GetValueByKeyIncludingId(bookData);
      const userIdValue = GetValueByKeyIncludingId(bookData);

      const User =
        (userList && userList?.filter((i) => i.name == selectedUser)[0]) ?? {};
      const UserId = GetValueByKeyIncludingId(User);
      const issueBookData = {
        userId: UserId,
        dueDate: date,
      };
      ...
      if (isBookIssue) {
        ...
        POST_API({
          endPoint: `${BOOK}/${"return"}/${bookIdValue}`,
          body: {},
        })
          .then((res) => {
            ToastSuccess(res);
